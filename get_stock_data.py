...
class Downloader(object):
    ...
    def run(self):
        ...
        for index, row in stock_df.iterrows():
            ...
            df_code.to_csv(f'{self.output_dir}\\{row["code"]}.{row["code_name"].replace("*", "star")}.csv', index=False)
        self.exit()
...
if __name__ == '__main__':
    ...
    downloader = Downloader('.\\stockdata\\train', date_start='1990-01-01', date_end='2022-11-10')
    downloader.run()

    mkdir('./stockdata/test')
    downloader = Downloader('.\\stockdata\\test', date_start='2019-12-01', date_end='2022-11-10')
    downloader.run()
